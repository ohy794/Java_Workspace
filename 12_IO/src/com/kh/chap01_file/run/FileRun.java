...
public class FileRun {
	...
	public static void main(String[] args) {
		...
		//1. 별도의 경로 지정을 하진 않고 파일면(test.txt)만 제시해서 생성해보기 => 경로는 현재 디렉토리
		...
		try {
			...
			//4. 별도의 경로 지정 없이 폴더 생성 후 파일 생성하기 => 프로젝트 폴더 내에 생성될 것
		...
		/*
		 * 프로그램 상의 데이터를 외부매체(모니터, 스피커, 자바프로그램, 파일 등등)로 출력하거나 입력장치(키보드, 마우스등)로
		 ...
		 * 스트림의 구분
		 * - 통로의 사이즈 :
		 * 		바이트 스트림 : 1byte짜리가 이동할 수 있을 정도의 사이즈(좁은통로) => 입력(XXXInputStream)/출력(XXXOutputStream)
		 * 		문자 스트림	  : 2byte짜리가 이동할 수 있을 정도의 사이즈(넓은통로) => 입력(XXXReader)		/출력(XXXWriter)
		 * 
		 * - 외부매체와의 직접적인 연결여부 :
		 * 		기반 스트림 : 외부 매체와 직접적으로 연결되는 통로
		 * 		보조 스트림 : 기반스트림만으로 부족한 성능을 향상시켜주는 용도의 스트림(직접적으로 외부매체와 연결할 수 없음, 즉 단독 사용이 불가)
		 * 				  (보조해주는 기능 : 속도향상, 자료형 반환, 객체단위의 입출력 보조 등)
		 * 				  단, 보조스트림은 단독적으로 사용이 불가능하므로 반드시 외부 매체와 직접적으로 연결가능한 기반 스트림과 함께 사용해야한다. 
		 * 
		 */
	}
