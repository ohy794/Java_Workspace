...
public class A_UncheckedException {
	 ...
	 * - NullPointerException : 래퍼런스가 아직 null임에도 불구하고 접근하려고 할 때 발생하는 예외
	 * - ArithmeticException : 나누기 연산시 0으로 나눌 때 발생하는 예외
	 * - ......
	 * 
	 * => 이러한 RuntimeException과 관련한 예외는 충분히 예상 가능한 상황이기 때문에
	...
	public void method1() {
		...
		//해결방법 1. 조건문으로 처리(애초에 예외자체가 발생되지 않게끔 IF문으로 조건검사하기) => 예외처리 X
//		if(num2 ==0) {
...
//			System.out.println("0으로 나눌 수 없습니다.");
	...
	public void method2() {
		...
		try {			
			...
			System.out.println("나눗셈 연산 결과 : " + (10 / num));//에러발생할 수 있음
		} catch(ArithmeticException e) {
			System.out.println("0으로 나눌 수 없습니다.");
	...
	public void method3() {
		// ArrayindexOutOfBoundException : 배열의 부적절한 인덱스로 접근시 발생하는 예외
		// NegativeArraySizeException : 배열의 크기를 음수로 지정할 경우 발생하는 예외
		System.out.print("배열의 크기 : " );
		int size = Integer.parseInt(sc.nextLine());
		
//		if(size > 0) {//방법 1. if문으로 size값이 양수값일때만 생성하도록 제어.
//			int[] arr = new int[Math.abs(size)];// 방법2. 항상 양의 정수값이 들어갈수 있도록 abs매서드 사용
//		}
...
//		}
		...
		}// 다중catch블럭이 줄어들었음. 하지만 정확히 어떤 에러인지는 알 수 없음.
	 ...
	 * - 예외처리 구문으로도 해결 가능 => 예외가 발생했을때를 대비해서 그 때 실행할 내용을 정의해두는것.
	 * 
