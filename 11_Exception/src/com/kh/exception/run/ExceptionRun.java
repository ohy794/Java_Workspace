...
public class ExceptionRun {
	public static void main(String[] args) {
		 ...
		 * 				=> 사용자의 잘못일수도 있고 개발자가 예측해야하는데 하지 못한경우
		 * 
		 * - 논리 에러 : 소스코드상으로 문제 없고, 실행했을 때도 에러가 발생하지 않지만 , 의도한대로 결과가 나오지 않는경우
		 * 
		 * 시스템에러를 제외한 컴파일에러, 런타임에러, 논리에러와 같은 비교적 덜 심각한 것들을 가지고 작업을 함.
		 * 이런 것들을 "예외"라고 부름.(Exception)
		 * 
		 * 이러한 예외들이 발생했을경우에 대비해서 처리하는 방법을 정의해두는것을 "예외처리" 라고 한다.
		 * 
		 * 예외 처리를 해야하는 이유 : 예외 발생 시 프로그램이 비정상적으로 종료되는것을 막기위함.
		 * 
		 * 예외처리방법
		 * 1. try ~ catch (직접처리)
		 * 2. throws 이용 (떠넘기기)
