...
public class Server {
	 ...
	 * 7) 스트림을 통해 입력 및 출력
	 * 8) 통신종료
	 *  - inputStream, OutputStream, Socket을 전부 닫아줘야함(Close)
	 */
	public void serverStart() {
		...
		try {
			...
			while(true) {
				//3) 접속 요청이 오면 요청 수락 후 해당 클라이언트에 대한 소켓객체 생성
				//4) 클라이언트의 정보를 저장
				System.out.println("클라이언트 대기중...");
				Socket client = server.accept();// Socket -> 클라이언트와 통신할 수 있는 객체
				System.out.println(++clientCnt +"클라이언트 연결됨...");
				...
				// 클라이언트에게 값을 한 줄 단위로 출력하는 스트림
				PrintWriter pw = new PrintWriter(os, true);// 데이터 출력할때 print(),println() 메서드를 사용
				...
				while(true) {
					...
					if(message ==null || "exit".equals(message)) {
						...
						pw.flush();//현재 스트림에 있는 데이터를 강제로 내보내는 메서드 close()메서드 호출되면 자동으로 flush 호출됨
					}
				}
				//8) 통신종료
				br.close();
