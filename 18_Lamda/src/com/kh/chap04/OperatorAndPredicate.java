...
public class OperatorAndPredicate {
	...
	public static void main(String[] args) {
		...
		pList.add(new Product("갤럭시 z폴드3",1050000,0));
		pList.add(new Product("아이폰 14",1850000,5));
		pList.add(new Product("갤럭시 2",350000,0));
		for(Product t:pList) {
			if(checkStock.test(t)) {//재고가 0개 보다 많다
				System.out.println(t.getpName() + "상품은 현재 재고가 있습니다.");
			}else {// 재고가 0개 이하
				System.out.println(t.getpName() + "상품은 현재 재고가 없습니다.");
			}
		}
	}
