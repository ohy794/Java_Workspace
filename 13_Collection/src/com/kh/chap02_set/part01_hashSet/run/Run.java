...
public class Run {
	...
	public static void main(String[] args) {
		...
		// 저장순서 유지 X, 중복 저장 O -> 동일객체로 판단되지 않기 때문
		// 값이 추가될 떄 equals(), hashcode()로 비교 후 둘다 결과가 true면 동일객체로 판단
		
		//HashSet에 객체를 담을 때 내부적으로 equals()로 비교 + hasgCode()값이 일치하는지도 비교
		// equals()한 결과가 true이고 hashCode()값이 일치하면 => 동일객체로 판단(중복저장X)
		
		// Object 클래스에 있는 equals() : 두 객체의 주소값을 가지고 비교해서 일치하면 true/ 일치하지않으면 false
		// Object 클래스에 있는 hashCode() : 두 객체의 주소값을 기반으로해서 해시코드값을 만들어서 변환하여 비교
		...
		// 1. for문 사용 가능 (단, for-each문(향상된for문)만 가능 => index로 접근이 불가하므로)
		...
		// 2. ArrayList에  담아중 후 ArrayList 반복문 수행
		...
		// 3. HashSet 클래스에서 제공하는 Iterator반복자를 이용한 방법
		// hs2에 담겨있는 객체들을 Iterator(반복자)에 담는 과정
		
		Iterator<Student> it = hs2.iterator();
		
		while(it.hasNext()) {// Iterator에서 it.next로 빼올 값이 있는가? 라고 물어보는 메소드
