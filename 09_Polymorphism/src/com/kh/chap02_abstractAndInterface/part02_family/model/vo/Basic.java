...
public interface Basic {
	 ...
	 * 
	 * - 상수필드와 추상메서드로만 이루어진 추상클래스의 변형체
	 * - 인터페이스에서 필드응 무 조 건 상수필드이다
	 * - 인터페이스에서 메소드응 무 조 겅 추상메서드이다
	 * - 무조건 구현해야하는게 있을 때 인터페이스를 만들어서 상속하게 된다
	 * - 인터페이스는 다중 상속을 허용한다.
	 *   (결국 메소드에 대한 정의만 하고 있어서 메소드가 겹치더라도 최종 구현 부분은 구현 클래스에서 이루어 질것이기 때문에
	 *   다중상속(구현)이 가능함.)
	 ...
	 *    인터페이스는 클래스의 기능(함수)구현을 강제 하기 위해 사용된다. 즉 구현을 강제함으로서 구현객체의 동일한 동작을
	 *    보장할 수 있다.
	 * 
	 * 
	 ...
	 *  - 인터페이스간에 상속 관계일 대 : 인터페이스명 extends 인터페이스명, 인터페이스명
	 * 
	 */
	
	/*public static final*/int NUM = 10; // 기울임글꼴 = static
	//인터페이스에는 상수필드만 정의할 수 있기 때문에
	//앞의 public static final 필드가 자동으로 붙어있게됨
	//num = 11; 상수라서 수정불가
	
	/*public abstract*/public void eat();
	void sleep();
	// 인터페이스에서는 추상 메소드만 정의할 수 있기 때문에 메소드의 몸통부 작성이 불가능하며
